...
- **ADR-15. Бэклинки на сообщения (M13).** К пунктам дайджеста добавляем ссылку на
  ...
  а модель сама решает, к каким буллетам его привязать; инструкция «сохраняй релевантную ссылку,
  не выдумывай» — в `formatRules`. Так нет битых/галлюцинированных ссылок (источник — наш код),
  но привязка к смыслу — на модели. **Красная линия prompt-cache (ADR-9/10):** URL живёт только
