...
Развилки (разобрать ДО кода → ADR-14):
- **«Флаг задан явно»** — ключевая проблема (ADR-3): нельзя отличить «не передан» от
  нуля/пустой строки. Варианты: stdlib `flag` + `flag.Visit` (применять только посещённые)
  или pointer-поля. Рекомендация: `flag.Visit`/pointer — строго применять лишь переданное.
- **Сосуществование с `conf`** — он уже парсит флаги секретов на тех же `os.Args`; два
  падающих на «unknown flag» парсера нельзя. Варианты: (а) расширить ту же conf-модель
  overrides и парсить один раз; (б) отдельный FlagSet, не конфликтующий с conf.
- **Набор полей**: все Settings или только скалярные? `source_chats` — повторяемый флаг
  vs CSV. `timezone`/`model`/`output_lang`/`max_output_tokens`/`window_hours`/`target_chat`.
- Имена — kebab (`--window-hours`); env-двойники для локала не обязательны.

- [ ] M11.1 Дизайн: набор оверрайдимых полей + механизм «только явно заданное» + способ
  сосуществования с `conf` (ADR-14).
- [ ] M11.2 `config`: применять overrides поверх yaml+defaults (только явно переданные;
  приоритет flag>yaml>default), не нарушая ADR-3.
- [ ] M11.3 `--help`: оверрайды видны в usage (или отдельная секция рядом с conf-флагами).
- [ ] M11.4 Тесты (чистые): применяются только заданные флаги; `0`/`""` как валидный
  override проходит; без флагов Settings не меняются (инвариант ADR-3).
- [ ] M11.5 README (EN+RU): пример локального разового запуска с флагами; отметить, что
  scheduled-прогон в Actions использует `config.yml`.
- [ ] M11.6 `go build/vet/test ./...`, `gofmt -l .` — зелёные. ADR-14.
...
(расширяет FR-9). Развилки/ADR-15: формат для приват (`t.me/c/...`) vs публичных
(`t.me/<user>/<msg>`); кто вставляет ссылку (модель по переданным id vs пост-обработка);
приватная ссылка откроется только участнику — задокументировать.

- [ ] M13.1 `telegram`: пробросить message id (+ chat id для `t.me/c`) в нормализацию/`Message`.
- [ ] M13.2 Способ вставки ссылок (ADR-15): через промпт с id vs пост-процессинг.
- [ ] M13.3 Формат под Telegram — голый URL (автолинк), без markdown (`formatRules`).
- [ ] M13.4 Тесты: чистая `linkFor(peer, msgID)`. README. `build/vet/test/gofmt` зелёные.
